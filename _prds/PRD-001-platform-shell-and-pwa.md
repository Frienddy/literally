...
| | |
|---|---|
| **Status** | **Done** — code + automated tests complete; real-device manual checks pending hardware (see §9) |
...
- [x] Project scaffolds (Vite + React 18 + TS strict + Tailwind); `npm run`
  `dev`/`build`/`preview`/`lint`/`test`/`test:e2e` all run.
- [x] Installable PWA: manifest (`standalone`, `portrait`, 192/512/maskable icons,
  theme/background `#0b1020`, `start_url`/`scope` `/`) + precaching service worker
  generated by `vite build`; SW precaches the shell incl. `/index.html` fallback.
- [x] Three-layer gesture blocking (CSS globals + viewport meta + `usePreventGestures`),
  portrait `PortraitGuard` + opportunistic lock, safe-area insets in `AppShell`,
  install affordance (`useInstallPrompt` + iOS guidance), `src/config.ts` tunables.
- [x] Playwright shell E2E passes on **Mobile Chrome + Mobile Safari**: frame is
  `position: fixed`, document can't scroll, selection disabled, `PortraitGuard`
  shows in landscape and content restores in portrait.
- [x] Production JS bundle ≈ 48 KB gzip — well under the ~200 KB NFR-3 budget.

**Real-device manual — pending hardware ⏳ (`_docs/09` §6; cannot be done in CI/emulator)**

- [ ] App installs to home screen on a **real iPhone and a real Android**.
- [ ] Loads in **airplane mode** after first visit.
- [ ] No pull-to-refresh, no text selection, no double-tap/pinch zoom; edge-swipe-back
  does not exit mid-game when installed; landscape shows `PortraitGuard`.
- [ ] Lighthouse PWA: installable + offline pass.
- [ ] The Phase 0 mobile-shell checklist in [05 §6](../_docs/05-pwa-and-mobile-shell.md)
  is fully green on device.
